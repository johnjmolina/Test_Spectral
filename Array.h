...
public:
  ...
  int Idx(int ix, int iy)const {return ix*ny + iy;}
...
public:
  ...
  int Idx(int ix, int iy, int iz)const {return ix*nyz + iy*nz + iz;}
...
public:
  ...
  int Idx(int ix, int iy, int iz, int iw)const {return ix*nyzw+iy*nzw+iz*nw+iw;}
...
public:
  ...
  int Idx(int ix, int iy, int iz, int iw, int iv)const {return ix*nyzwv+iy*nzwv+iz*nwv+iw*nv+iv;}
...
// b <- a
template<typename T>
void ForceLoad(array2<T> &b, const array2<T> &a){
  unsigned int n1, n2;
  n1 = a.Nx();
  n2 = a.Ny();
#pragma omp parallel for
  for(unsigned int i = 0; i < n1; i++){
    _array1<T> ai = a[i];
    _array1<T> bi = b[i];
    for(unsigned int j = 0; j < n2; j++) bi[j] = ai[j];
  }
}
// b -> a
template<typename T>
void ForceStore(const array2<T> &b, array2<T> &a){
  unsigned int n1, n2;
  n1 = a.Nx();
  n2 = a.Ny();
#pragma omp parallel for  
  for(unsigned int i = 0; i < n1; i++){
    _array1<T> ai = a[i];
    _array1<T> bi = b[i];
    for(unsigned int j = 0; j < n2; j++) ai[j] = bi[j];
  }
}
// b <- a
template<typename T>
void ForceLoad(array3<T> &b, const array3<T> &a){
...
#pragma omp parallel for  
  for(unsigned int i = 0; i < n1; i++){
    ...
    for(unsigned int j = 0; j < n2; j++){
      _array1<T> aij = ai[j];
      _array1<T> bij = bi[j];
      for(unsigned int k = 0; k < n3; k++) bij[k] = aij[k];
    }
  }
}
// b -> a
template<typename T>
void ForceStore(const array3<T> &b, array3<T> &a){
  unsigned int n1, n2, n3;
  n1 = a.Nx();
  n2 = a.Ny();
  n3 = a.Nz();
#pragma omp parallel for  
  for(unsigned int i = 0; i < n1; i++){
    ...
    for(unsigned int j = 0; j < n2; j++){
      _array1<T> aij = ai[j];
      _array1<T> bij = bi[j];
      for(unsigned int k = 0; k < n3; k++) aij[k] = bij[k];
    }
  }
}
}
